...
def downgrade() -> None:
    op.execute(
        """
DROP TABLE IF EXISTS eval_runs, idempotency_keys, chunks, documents, level_events, messages, conversations, skill_levels, users CASCADE;
DROP TYPE IF EXISTS idempotency_status, response_kind, level_source, explanation_mode, intent_type, message_role, user_level;
"""
    )
