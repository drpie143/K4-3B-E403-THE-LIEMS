...
class AuthStore:
    ...
    def login(self, email: str, password: str) -> tuple[dict, str]:
        ...
        with self.lock:
            self.db.execute("INSERT OR REPLACE INTO account_sessions VALUES (?,?,?,?)",
                            (row["token_hash"], row["account_id"], row["created_at"], row["expires_at"]))
            self.db.execute("UPDATE accounts SET last_login_at=? WHERE id=?", (iso(now), acc["id"]))
            self.db.commit()
        if self.remote:
            self.sb.upsert("account_sessions", row)
            self.sb.update("accounts", {"id": f"eq.{acc['id']}"}, {"last_login_at": iso(now)})
        return self._public(acc), token

    def logout(self, token: str) -> None:
        th = token_hash(token or "")
        with self.lock:
            self.db.execute("DELETE FROM account_sessions WHERE token_hash=?", (th,))
            self.db.commit()
        if self.remote:
            self.sb.delete("account_sessions", {"token_hash": f"eq.{th}"})

    def account_for_token(self, token: str | None) -> dict | None:
        if not token:
            return None
        th = token_hash(token)
        row = self.db.execute("SELECT account_id, expires_at FROM account_sessions WHERE token_hash=?", (th,)).fetchone()
        if row is None and self.remote:
            rows = self.sb.select("account_sessions", {"token_hash": f"eq.{th}", "select": "account_id,expires_at"})
    ...
    def update_account(self, account_id: str, display_name: str | None = None, password: str | None = None) -> dict:
        ...
        if patch:
            ...
            with self.lock:
                self.db.execute(f"UPDATE accounts SET {sets} WHERE id=?", (*patch.values(), account_id))
                self.db.commit()
            if self.remote:
                self.sb.update("accounts", {"id": f"eq.{account_id}"}, patch)
            acc = {**acc, **patch}
        return self._public(acc)
    ...
    def delete_account(self, account_id: str) -> None:
        with self.lock:
            ...
            self.db.commit()
        if self.remote:
            self.sb.delete("account_sessions", {"account_id": f"eq.{account_id}"})
            self.sb.delete("accounts", {"id": f"eq.{account_id}"})
    ...
    def _find(self, email: str) -> dict | None:
        ...
        if row:
            return dict(row)
        if self.remote:
            rows = self.sb.select("accounts", {"email": f"eq.{email}", "select": "*"})
            if rows:
                self._cache(rows[0])
                return rows[0]
        return None
    ...
    def _by_id(self, account_id: str) -> dict | None:
        ...
        if row:
            return dict(row)
        if self.remote:
            rows = self.sb.select("accounts", {"id": f"eq.{account_id}", "select": "*"})
            if rows:
                self._cache(rows[0])
                return rows[0]
        return None
