...
def test_mat_khau_khong_luu_dang_tho(orch):
    ...
    row = orch.store.db.execute("SELECT password_hash FROM accounts WHERE email=?", ("c@vinuni.edu.vn",)).fetchone()
...
def test_http_dang_nhap_roi_hoc(orch):
    ...
    assert orch.store.db.execute("SELECT 1 FROM settings WHERE user_id=?", ("nguoi-khac",)).fetchone() is None
...
def test_xoa_tai_khoan_xoa_luon_du_lieu(orch):
    ...
    assert orch.store.db.execute("SELECT 1 FROM accounts WHERE id=?", (uid,)).fetchone() is None
    assert orch.store.db.execute("SELECT 1 FROM profiles WHERE user_id=?", (uid,)).fetchone() is None
