...
Việc script làm:
  1. Gọi build_local_data.py để sinh codebase/data/chunks.local.json (KHÔNG commit)
...
Cách chạy (từ gốc repo):
    python codebase/scripts/build_lessons.py
    python codebase/scripts/build_lessons.py --pack "/đường/dẫn/data/vlearn-pack"
    python codebase/scripts/build_lessons.py --check     # chỉ kiểm tra, không sinh lại dữ liệu
"""
...
def main() -> int:
    ...
    if not args.check:
        cmd = [sys.executable, str(CODEBASE / "scripts" / "build_local_data.py")]
        if args.pack:
            cmd += ["--pack", args.pack]
    ...
    print("  Dữ liệu nguyên văn nằm ở codebase/data/chunks.local.json (đã gitignore).")
    print("  Muốn đẩy lên Supabase: python codebase/scripts/sync_to_supabase.py")
    return 0
