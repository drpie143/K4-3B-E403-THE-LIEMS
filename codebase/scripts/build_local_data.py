...
Đầu ra (đều đã nằm trong .gitignore):
  codebase/mock/data/sources.local.js     nguyên văn các đoạn transcript mà mock trích dẫn
  codebase/data/chunks.local.json         toàn bộ đoạn transcript-04/06 (chuẩn bị cho retrieval ở CP3)
  codebase/eval/golden_candidates.local.csv  các lượt chatlog K4 làm ứng viên golden set

Cách chạy (từ gốc repo):
  python codebase/scripts/build_local_data.py
  python codebase/scripts/build_local_data.py --pack "/đường/dẫn/tới/data/vlearn-pack"
...
def referenced_ids():
    src = (CODEBASE / "mock" / "js" / "content.js").read_text(encoding="utf-8")
...
def write_chunks(chunks):
    path = CODEBASE / "data" / "chunks.local.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def write_golden(pack):
    ...
    path = CODEBASE / "eval" / "golden_candidates.local.csv"
    path.parent.mkdir(parents=True, exist_ok=True)
