...
(function (root) {
  ...
  $("#lessonBody").addEventListener("mouseup", () => {
    setTimeout(() => {
      const sel = window.getSelection();
      const text = sel ? sel.toString().trim() : "";
      $("#tooLong").hidden = true;
      if (!text) { pop.style.display = "none"; return; }
      if (text.length > 280) { pop.style.display = "none"; $("#tooLong").hidden = false; return; }
  ...
  document.addEventListener("mousedown", (e) => { if (e.target !== pop && !pop.contains(e.target)) pop.style.display = "none"; });
