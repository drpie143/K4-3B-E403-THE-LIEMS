...
(function () {
  ...
  function selectLesson(idx) {
    ...
    const dayEl = $("#lessonDay");
    if (dayEl) dayEl.textContent = l.day;

    const progEl = $("#lessonProgress");
    if (progEl) progEl.innerHTML = (idx + 1) + "/" + list.length + " bài <div class='bar' style='width:" + (((idx + 1) / list.length) * 100) + "%'></div>";

    const crumbEl = $("#lessonCrumb");
    if (crumbEl) crumbEl.innerHTML = "<i></i>" + l.crumb;

    const titleEl = $("#lessonTitle");
    if (titleEl) titleEl.textContent = l.title;

    const tagEl = $("#lessonTag");
    if (tagEl) tagEl.textContent = l.tag;

    const sTitle = $("#slideTitle");
    if (sTitle) sTitle.textContent = l.title;

    const sSub = $("#slideSub");
    if (sSub) sSub.textContent = l.crumb;

    const sCont = $("#slideContent");
    if (sCont) sCont.textContent = l.summary;

    const kpList = $("#keyPointsList");
    if (kpList && l.keyPoints) {
      kpList.innerHTML = l.keyPoints.map((p) => "<li>" + p + "</li>").join("");
    }

    renderSidebar();
    greeting();
    toast("Đã chuyển sang: " + l.day);
  }
  ...
  function renderSidebar() {
    ...
    list.forEach((l, i) => {
      ...
      const btn = h("button", {
        ...
        h("span", { class: "label", text: l.title }),
        isDone ? h("span", { class: "state done" }, [icon("i-check", 14), " Đã học"]) : null,
        isActive ? h("span", { class: "state", text: "Đang học" }) : null,
      ]);
      box.appendChild(btn);
    });
  }
  ...
  function greeting() {
    ...
    chat.innerHTML = "";
    const list = C.REAL_LESSONS || [C.LESSON];
    const l = list[currentLessonIndex] || list[0];
    const starters = l.starters || [
      "Bài học này tập trung vào vấn đề gì?",
      "Các khái niệm quan trọng nhất trong bài là gì?",
      "Giải thích tóm tắt bài này theo cách đơn giản nhất.",
    ];
    chat.appendChild(
      h("div", { class: "empty" }, [
        h("div", { class: "hi", text: "Chào buổi " + part + ", " + DEMO_USER.name.toUpperCase() + "!" }),
        h("div", { class: "ctx", text: "Đang mở: " + l.title }),
  ...
  function typing() {
    const el = h("div", { class: "msg ai" }, [h("div", { class: "ai-ico" }, [icon("i-spark", 24)]), h("div", { class: "typing" }, [h("i"), h("i"), h("i")])]);
    chat.appendChild(el);
    scrollDown();
    return el;
  }
  ...
  function openSource(id) {
    ...
    if (!local) box.appendChild(h("p", { class: "muted", html: "Đang hiện <b>tóm tắt</b>. Chạy <code>python codebase/scripts/build_local_data.py</code> để xem nguyên văn trên máy (file sinh ra không được commit)." }));
    openOverlay("#sourceView");
  }
  ...
  if (lb) {
    lb.addEventListener("mouseup", () => {
      setTimeout(() => {
        ...
        if (!text) { if (pop) pop.style.display = "none"; return; }
        if (text.length > 280) { if (pop) pop.style.display = "none"; if (tl) tl.hidden = false; return; }
        if (!sel || !sel.rangeCount) return;
        const rect = sel.getRangeAt(0).getBoundingClientRect();
        if (pop) {
          pop.style.display = "inline-flex";
          pop.style.top = Math.max(64, rect.top - 42) + "px";
          pop.style.left = Math.min(window.innerWidth - 180, rect.left + rect.width / 2 - 80) + "px";
          pop.dataset.text = text;
        }
      }, 10);
    });
  }
  document.addEventListener("mousedown", (e) => { if (pop && e.target !== pop && !pop.contains(e.target)) pop.style.display = "none"; });
  if (pop) {
    pop.addEventListener("click", () => {
      S.selection = pop.dataset.text || "";
      const st = $("#selText");
      if (st) st.textContent = "Đoạn đã chọn: “" + S.selection + "”";
      const sc = $("#selChip");
  ...
  const scBtn = $("#selClear");
  if (scBtn) scBtn.addEventListener("click", clearSelection);
