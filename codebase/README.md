...
|---|---|---|
| `mock/` | Giao diện trang học VLearn + khung Trợ giảng AI | Bấm đúp `mock/index.html` (logic bằng luật) hoặc mở qua backend để dùng AI thật |
...
# (2a) Chỉ mock: bấm đúp codebase/mock/index.html
...
# (3) Test
node --test codebase/tests/engine.test.js                # 16 test logic của mock
codebase/backend/.venv/bin/python -m pytest -q codebase/backend   # 40 test backend (FakeLLM)

# (4) Đánh giá golden set
codebase/backend/.venv/bin/python codebase/eval/run_eval.py --round 1 --split test
codebase/backend/.venv/bin/python codebase/eval/run_eval.py --round 1 --split test --baseline
```
...
**Quy tắc dữ liệu:** mọi thứ lấy từ data pack chỉ nằm trong file `*.local.*` hoặc thư mục `data/` — đều bị `.gitignore` chặn. Trong repo chỉ có **mã đoạn** (`T06-130`, `T10728`) và diễn giải nhóm tự viết. Trước khi commit, chạy `bash codebase/scripts/check_no_data.sh` và `git status --ignored`.
