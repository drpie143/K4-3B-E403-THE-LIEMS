...
(function (root) {
  ...
  function setAccount(acc) {
    ...
    // Chưa đăng nhập: nút "Đăng nhập" có chữ, dễ thấy. Đã đăng nhập: avatar mở menu tài khoản.
    if (btn) {
      btn.textContent = acc ? initial(acc) : "?";
      btn.classList.toggle("signed", !!acc);
      btn.hidden = !acc;
      btn.title = acc ? acc.display_name + " · " + acc.email : "Tài khoản của bạn";
    }
    if (signIn) signIn.hidden = !!acc;
  ...
  function openAuth(tab) {
    ...
    function render() {
      ...
      const label = mode === "login" ? "Đăng nhập" : "Tạo tài khoản";
      const submitBtn = h("button", { class: "btn primary", type: "submit", text: label });
      ...
      async function submit() {
        err.hidden = true;
        // Lượt đầu phải kéo hồ sơ từ Supabase về nên có thể mất vài giây — phải cho thấy
        // là đang chạy, không thì học viên tưởng bấm hụt và bấm lại.
  ...
  async function boot() {
    const btn = $("#topAvatar");
    const signIn = $("#signInBtn");
    if (signIn) signIn.addEventListener("click", () => openAuth("login"));
    if (btn) {
