...
class GeminiClient:
    ...
    def __init__(self, timeout: float = 20.0, max_retries: int = 2):
        ...
        # Gemini 2.5 mặc định BẬT "thinking" — mỗi lượt gọi chậm thêm vài giây mà ba việc ở đây
        # (chẩn đoán / giải thích / chấm) đều đã có luật và schema chặt, không cần suy luận dài.
        # Đặt GEMINI_THINKING_BUDGET=-1 để trả về mặc định của Google, hoặc số token cụ thể.
    ...
    def complete_json(self, task, system, user, schema, model, fallback=None):
        ...
        if self.thinking_budget >= 0:
            try:
                config.thinking_config = self.types.ThinkingConfig(thinking_budget=self.thinking_budget)
            except (AttributeError, TypeError):
                pass  # model/SDK không hỗ trợ tắt thinking (vd. gemini-2.5-pro) → bỏ qua
