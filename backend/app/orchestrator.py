...
from .fidelity import WORD_TARGET, fake_judge, merge_judge, plain_text, rule_check
...
class Turn:
    ...
    memory: dict = field(default_factory=dict)  # nhịp ghi bộ nhớ dài hạn của lượt này
    speculative_hit: bool = False  # bản soạn sẵn song song với chẩn đoán có dùng được không
    judge_skipped: bool = False    # bỏ bước LLM chấm vì câu trả lời toàn phần đã duyệt
    marks: dict = field(default_factory=dict)  # mốc thời gian từng chặng, để soi chỗ nào chậm

    def mark(self, name: str) -> None:
        """Ghi mốc: bao nhiêu mili giây kể từ đầu lượt. Xem trong traces/<ngày>.jsonl."""
        self.marks[name] = int((time.perf_counter() - self.t0) * 1000)
...
class Orchestrator:
    ...
    def _trace(self, turn: Turn, route: str, req, resp: ChatResponse | None = None, extra: dict | None = None) -> None:
        rec = {
            ...
            "speculative_hit": turn.speculative_hit,
            "judge_skipped": turn.judge_skipped,
            "marks_ms": turn.marks,
    ...
    # ================================================================ responses
    def _suggestions_for(self, lesson_id: str) -> list[str]:
        """Gợi ý hỏi tiếp — lấy khái niệm của ĐÚNG buổi đang mở.

        Trước đây là hằng số cố định từ hồi chỉ có một buổi, nên khi trợ giảng từ chối
        "Self-attention" ở buổi 1 thì lại gợi ý ngay chính câu vừa từ chối.
        """
        terms = [self.cards.get(c).term for c in self.lesson_concepts(lesson_id) if self.cards.get(c)]
        return [f"{t} là gì?" for t in terms[:3]] or list(SUGGESTIONS)

    def _lesson_with_concept(self, concept_id: str) -> tuple[str, str] | None:
        """Khái niệm này nằm ở buổi nào? → (tên buổi, tiêu đề) để chỉ đường cho học viên."""
        for lid, meta in self.cards.lessons.items():
            if meta.get("order") and concept_id in (meta.get("concepts") or []):
                return meta.get("day", ""), meta.get("title", lid)
        return None

    def _card_for_term(self, text: str) -> Card | None:
        """Dò xem câu hỏi nhắc tới thẻ khái niệm nào trong cả khoá (không chỉ buổi đang mở)."""
        n = _norm(text)
        best = None
        for card in self.cards.cards.values():
            for name in (card.term, card.vi_name, card.id.replace("_", " ")):
                cand = _norm(name or "")
                if len(cand) > 3 and cand in n and (best is None or len(cand) > len(best[0])):
                    best = (cand, card)
        return best[1] if best else None

    def _scope(self, turn: Turn, kind: str, text: str, term: str | None = None, nearest: list[str] | None = None,
               decision: Decision | None = None, in_lesson: bool = False, lesson_id: str = "") -> ChatResponse:
        lesson_name = self.lesson_title(lesson_id) or "buổi đang mở"
        if kind == "injection":
            msg = ("Tin nhắn có yêu cầu thay đổi hướng dẫn của trợ giảng — mình coi đó là nội dung và không làm theo. "
                   f"Mình chỉ giúp giải thích nội dung buổi đang học ({lesson_name}).")
            ...
            msg = f"{name} chưa có trong bài giảng buổi này ({lesson_name}), nên mình không giải thích để tránh đoán sai."
            # Khái niệm có trong khoá nhưng ở buổi khác → chỉ đường thay vì để học viên tắc ở đây.
            other = self._card_for_term(text)
            where = self._lesson_with_concept(other.id) if other else None
            if where:
                msg = (f"{other.term} thuộc buổi khác của khoá — {where[0]} · {where[1]}. "
                       f"Bạn mở buổi đó ở cột trái rồi hỏi lại, mình sẽ giải thích kèm đoạn gốc.")
        else:
            msg = (f"Mình là trợ giảng cho buổi đang mở ({lesson_name}). Bạn có thể bôi đen một đoạn trong bài rồi hỏi, "
                   "hoặc hỏi thẳng một khái niệm trong bài.")
        d = decision or Decision(kind=kind, in_scope=kind in ("no_source", "help"), confidence=0.9)
        return ChatResponse(kind=kind, decision=d,
                            scope=ScopePayload(message=msg, term=term, nearest=nearest or [],
                                               suggestions=self._suggestions_for(lesson_id)),
    ...
    def _explain_key(self, d: Decision) -> str:
        """Khoá nhận dạng một lời giải thích: hai quyết định cùng khoá thì cho ra cùng bài."""
        return AnswerCache.key(
            concept=d.concept, ask=d.ask_type, level=d.level, style=d.style, prereq=d.prereq_first,
            analogy=d.preferred_analogy, alt=d.alt_example, full=d.full, ws=d.weighted_sum,
            prompt=self.prompts.version, model=self.s.model_explain, provider=self.provider,
        )

    def _diagnose_and_explain(self, turn: Turn, ctx: Ctx, rules: Decision, text: str, selection: str,
                              hits: list[Passage], note: str = "") -> tuple[Decision, tuple[Answer, FidelityReport] | None]:
        """Chẩn đoán (LLM) và soạn bài theo luật — chạy song song vì không phụ thuộc nhau.

        Phần lớn lượt hỏi, LLM chốt đúng mức/kiểu mà luật đã đề xuất; khi đó bản soạn sẵn dùng
        được luôn và ta tiết kiệm trọn một vòng gọi mô hình (~4–6 giây). Khi LLM chốt khác,
        bản soạn sẵn bị bỏ và `_explain_response` soạn lại theo quyết định thật.
        """
        if not self.s.speculative_explain or self.s.replay or rules.kind != "explain":
            return self._diagnose(turn, ctx, rules, text, selection, hits), None
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="turn") as pool:
            f_dec = pool.submit(self._diagnose, turn, ctx, rules, text, selection, hits)
            f_ans = pool.submit(self._explain, turn, ctx, rules, text, selection, hits, note)
            d = f_dec.result()
            try:
                spec = f_ans.result()
            except Exception:       # bản soạn sẵn hỏng thì lượt hỏi vẫn đi tiếp bình thường
                spec = None
        if spec and d.kind == "explain" and self._explain_key(d) == self._explain_key(rules):
            turn.speculative_hit = True
            return d, spec
        return d, None
    ...
    def _explain(self, turn: Turn, ctx: Ctx, d: Decision, text: str, selection: str, hits: list[Passage],
        ...
        fix = ""
        lo, hi = WORD_TARGET.get(d.level, (180, 300))
        length_hint = (f"nhắm khoảng {lo}–{hi} từ (không tính bảng, phần nền và phần ngoài bài). "
                       f"Nếu chưa tới {lo} từ, hãy khai thác thêm chi tiết trong <passages> và core_claims "
                       f"— tuyệt đối không kéo dài bằng câu nhận định không có trong nguồn.")
        for attempt in range(2):
            ...
            approved = self._approved_texts(card, template, pre)
            answer_for_judge = self._answer_text_for_judge(ans.blocks, approved)
            # Câu trả lời chỉ gồm phần đã có người duyệt (mẫu, ví dụ, primer) thì không có gì
            # để chấm — bỏ lượt gọi LLM chấm, tiết kiệm ~2–3 giây mỗi lượt.
            novel = "[tự viết" in answer_for_judge or "[NGOÀI BÀI]" in answer_for_judge
            if rep.ok and self.s.use_judge and self.s.judge_only_when_novel and not novel:
                turn.judge_skipped = True
                rep.judge_verdict = "skipped"
            elif rep.ok and self.s.use_judge:
    ...
    def _explain_response(self, turn: Turn, req, ctx: Ctx, d: Decision, text: str, selection: str,
                          hits: list[Passage], sess: dict, note: str = "",
                          precomputed: tuple[Answer, FidelityReport] | None = None) -> ChatResponse:
        answer, rep = precomputed or self._explain(turn, ctx, d, text, selection, hits, note)
        card = ctx.card
        now_ts = time.time()
    ...
    def chat(self, req: ChatRequest, survey: dict | None = None, turn: Turn | None = None) -> ChatResponse:
        ...
        if not sig.concept:
            ...
            if mapped:
                ...
                resp = self._scope(turn, kind, text, term=_guess_term(text) if kind == "no_source" else None,
                                   nearest=[h.id for h in strong[:2]], in_lesson=bool(strong), lesson_id=req.lesson_id)
                self.store.save_session(req.session_id, req.user_id, sess)
                self._trace(turn, "chat", req, resp)
                return resp
        ...
        d, spec = self._diagnose_and_explain(turn, ctx, rules, text, selection, hits, note)
        turn.mark("diagnose_explain")

        if d.kind == "out_of_scope":
            resp = self._scope(turn, "out_of_scope", text, decision=d, lesson_id=req.lesson_id)
        elif d.kind == "survey":
            ...
            resp = self._explain_response(turn, req, ctx, d, text, selection, hits, sess, note=note, precomputed=spec)
        turn.mark("response")
        self._trace(turn, "chat", req, resp)
        return resp
    ...
    def health(self) -> dict:
        return {
            ...
            "retrieval_mode": self.retriever.mode, "search_mode": getattr(self.retriever, "retrieval_mode", "bm25"),
            "passages": len(self.retriever.docs),
