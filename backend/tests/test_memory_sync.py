...
def test_khong_ghi_moi_luot_ma_gom_lai(orch):
    ...
    assert "person_profiles" in tables or "person_events" in tables
...
def test_hang_doi_khong_phinh_theo_so_luot(orch):
    ...
    assert len(orch.store.mem.queue["person_profiles"]) == 1
...
def test_nen_bo_nho_giu_toi_da_n_cach_giai_thich(orch):
    ...
    rows = orch.store.db.execute("SELECT strategy FROM person_strategy_memory WHERE user_id=?", ("u3",)).fetchall()
    assert len(rows) == 3
...
def test_nen_bo_cach_giai_thich_qua_han(orch):
    ...
    left = [r["strategy"] for r in orch.store.db.execute(
        "SELECT strategy FROM person_strategy_memory WHERE user_id=?", ("u4",)).fetchall()]
...
def test_nen_cat_nhat_ky_su_kien(orch):
    ...
    n = orch.store.db.execute("SELECT COUNT(*) c FROM person_events WHERE user_id=?", ("u5",)).fetchone()["c"]
    assert n <= 5
...
def test_nap_lai_ho_so_tu_supabase(orch):
    ...
    use_fake(orch, {
        "person_settings": [{"user_id": "u6", "memory_on": 1, "preferred_style": "vi_du"}],
        "person_profiles": [{"user_id": "u6", "concept": "self_attention", "level": "hieu_ro", "streak": 2, "source": "kiem_tra"}],
        "person_strategy_memory": [{"user_id": "u6", "concept": "self_attention", "strategy": "analogy:thu_vien", "worked": 2, "failed": 0}],
    })
...
def test_xoa_ho_so_xoa_ca_tren_supabase(orch):
    ...
    assert any(t == "person_profiles" for t, _ in sb.deletes)
    assert any(t == "person_strategy_memory" for t, _ in sb.deletes)
